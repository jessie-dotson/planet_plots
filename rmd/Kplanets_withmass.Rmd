...
```{r}
planets_m$source <- rep("All Others", nrow(planets_m))
...
p1 <- baseplot + geom_point(aes(x=fpl_bmasse, y=fpl_rade,col=source),alpha=0.5)
...
p2 <- baseplot + geom_bar(aes(x=size, fill=size))
print(p2)

p2 <- p2 + facet_wrap(~source)
p2 <- p2 + theme(axis.text.x = element_text(angle = 90)) + theme(legend.position = "none")
p2 <- p2 + ylab("Number with measured mass and radius")
print(p2)
