...
T
```{r}
p_planets$facility <- rep("not Kepler", nrow(p_planets))
jj <- which(p_planets$pl_instrument == "Kepler CCD Array")
p_planets$facility[jj] <- "Kepler"
...
p_planets <- add_row(p_planets,pl_disc=1990, facility="not Kepler")
p_planets <- add_row(p_planets,pl_disc=1991, facility="not Kepler")
p_planets <- add_row(p_planets,pl_disc=1993, facility="not Kepler")
...
```{r}
p_planets <- add_row(p_planets, pl_disc=1989:2008, facility="Kepler")
```
...
jj <- which(orig_p_planets$facility == "Kepler")
kepcount <- count(orig_p_planets[jj,], pl_disc)
kepcount <- mycum(kepcount)
p_planets2 <- add_row(p_planets2, pl_disc=1989:2008,facility="Kepler", count=0)
p_planets2 <- add_row(p_planets2, pl_disc=kepcount$pl_disc,facility="Kepler",count=kepcount$cum)

jj <- which(orig_p_planets$facility == "not Kepler")
notcount <- count(orig_p_planets[jj,], pl_disc)
notcount <- mycum(notcount)
p_planets2 <- add_row(p_planets2, pl_disc=1990:1991, facility="not Kepler",count=1)
p_planets2 <- add_row(p_planets2, pl_disc=1993, facility="not Kepler", count=3)
p_planets2 <- add_row(p_planets2, pl_disc=notcount$pl_disc, facility="not Kepler", count=notcount$cum)
...
thisplot <- thisplot + transition_time(pl_disc) + labs(title="Year: {floor(frame_time)}")
thisplot <- thisplot + shadow_mark(past=TRUE, col="white",fill="darkgrey",exclude_layer=2) 
thisplot <- thisplot + enter_appear()
thisplot <- thisplot + theme(plot.title=element_text(size=18),
                             axis.title.x=element_text(size=14),
                             axis.title.y=element_text(size=14),
...
colnames(temp) <- c("year","earth","superearth","neptune","jupiter","larger")

size_history_long <- gather(temp, size, count, earth:larger)


size_history_long$size_f <- factor(size_history_long$size, levels=c("earth","superearth","neptune","jupiter","larger"))
...
```{r}
krad_planets <- rad_planets %>% filter(facility == "Kepler")
...
ksize_history_long <- gather(temp, size, count, earth:larger)


ksize_history_long$size_f <- factor(ksize_history_long$size, levels=c("earth","superearth","neptune","jupiter","larger"))

ksize_history_long$facility <- "Kepler"

```


```{r}
nkrad_planets <- rad_planets %>% filter(facility == "not Kepler")
...
nksize_history_long <- gather(temp, size, count, earth:larger)


nksize_history_long$size_f <- factor(nksize_history_long$size, levels=c("earth","superearth","neptune","jupiter","larger"))

nksize_history_long$facility <- " not Kepler"
...
#animate(p1, nframes=300, width=1200, height=800, res=144, renderer=av_renderer())  #makes mp4         
animate(p1, nframes=300, width=500, height=500, end_pause=30) #makes gif
```
