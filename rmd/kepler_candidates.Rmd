...
```{r}
plot3 <- baseplot + geom_point(aes(x=koi_insol,y=koi_steff,col=size),alpha = 0.5)
...
templot <- ggplot(my_HZ_limits) + theme_bw()
templot <- templot + geom_ribbon(aes(x=T, ymin=recent_venus, ymax=early_mars),fill="lightgrey")
templot <- templot + geom_point(data=candidates, aes(x=koi_steff, y=koi_insol, col=size), alpha=0.5)
print(templot)
